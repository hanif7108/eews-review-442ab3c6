...
4. **Multi-station fusion via MQTT pub/sub architecture:** Network-based ensemble combining Stage 2 predictions from multiple nearby stations using FinDer-style source characterization [88] and PLUM-style direct shaking propagation. Böse et al. [72] original FinDer implementation provides the foundational architecture for such extension. The MQTT (Message Queuing Telemetry Transport) protocol is a strong candidate transport layer: it enables each station to publish its own IDA-PTW outputs (Stage 0 URPD probability, Stage 1 intensity class, Stage 1.5 distance estimate, Stage 2 spectral prediction) to a regional broker, with subscribers receiving cross-station fused alerts within ~25–280 ms additional latency — still faster than S-wave propagation for target distances ≥ 10 km. With guaranteed internet availability on the IA-BMKG fiber backbone, a hybrid MQTT-based architecture is expected to reduce $\sigma_{total}$ by 20–35% (0.755 → ~0.55–0.60), reduce false-alarm rate by 100–1000× via network voting, and enable effective zero blind zone for any target that has at least one upstream station in the network. This hybrid regional–on-site architecture — combining IDA-PTW on-site predictions with multi-station MQTT fusion — is the subject of a follow-up publication in preparation.
