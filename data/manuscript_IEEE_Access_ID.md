...
4. **Fusion multi-stasiun via arsitektur pub/sub MQTT:** Ensemble berbasis-jaringan yang menggabungkan prediksi Stage 2 dari banyak stasiun berdekatan menggunakan karakterisasi sumber gaya-FinDer [88] dan propagasi goyangan langsung gaya-PLUM. Implementasi FinDer asli Böse dkk. [72] menyediakan arsitektur fondasi untuk perluasan ini. Protokol MQTT (Message Queuing Telemetry Transport) adalah kandidat transport layer yang kuat: memungkinkan setiap stasiun mem-publish output IDA-PTW-nya (probabilitas Stage 0 URPD, kelas intensitas Stage 1, estimasi jarak Stage 1.5, prediksi spektral Stage 2) ke broker regional, dengan subscriber menerima alert terfusi lintas-stasiun dalam tambahan latensi ~25–280 ms — masih lebih cepat dari propagasi gelombang-S untuk target jarak ≥ 10 km. Dengan jaminan ketersediaan internet pada backbone fiber IA-BMKG, arsitektur hybrid berbasis MQTT diharapkan mengurangi $\sigma_{total}$ sebesar 20–35% (0,755 → ~0,55–0,60), mengurangi tingkat false-alarm 100–1000× via voting jaringan, dan memungkinkan efektif zona buta nol untuk target yang memiliki setidaknya satu stasiun upstream dalam jaringan. Arsitektur hybrid regional–on-site ini — mengombinasikan prediksi on-site IDA-PTW dengan fusion multi-stasiun MQTT — adalah subjek publikasi follow-up yang sedang dipersiapkan.
